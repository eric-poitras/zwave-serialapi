...
const { retryWhen, delay, map, mergeMap, first } = require('rxjs/operators')
...
function controllerChannel (serialApi) {
  ...
  commands.pipe(
    mergeMap(command => sendData(serialApi, command.command).then(result => [result, command.response]), 1)
  ).subscribe(res => {
    res[1].next(res[0])
    res[1].complete()
  })
